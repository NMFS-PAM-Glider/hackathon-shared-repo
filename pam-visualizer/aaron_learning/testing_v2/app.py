...
def main() -> None:
    ...
    st.caption(
        ...
        + "Your zoom is kept when you change transform; it resets when you change "
          "file, window or view, and double-clicking resets it to the whole "
          "loaded window."
    )

    # 5. deployment context ------------------------------------------------
    # Only in the Spectrogram view: the LTSA is frequency against time, so it
    # is the one view whose axes mean the same thing as the panels above.
    if view == "Spectrogram":
