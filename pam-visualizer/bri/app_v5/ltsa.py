...
def _to_stamp(date_field: str, time_field: str):
    """Build a Timestamp from a YYMMDD or YYYYMMDD date and an HHMMSS time."""
    if not (date_field.isdigit() and time_field.isdigit()):
        return None
    if len(time_field) != 6:
        return None

    if len(date_field) == 8:
        year, mm, dd = int(date_field[:4]), int(date_field[4:6]), int(date_field[6:8])
    elif len(date_field) == 6:
...
def parse_audio_time(name):
    """
    UTC start time from an audio filename. Handles the naming schemes in use
    across the fleet, trying each in turn:

        stenella_20260128_WISPR_260207_160803.flac  -> 2026-02-07 16:08:03
            date is the second-to-last underscore field, time is the last

        sea117.26_20260208-061638.wav               -> 2026-02-08 06:16:38
            last field is date-time joined by a hyphen

        sg607_20260208_061638.wav                   -> 2026-02-08 06:16:38
            four-digit year in the second-to-last field

    Returns a pandas Timestamp, or None if nothing matches.
    """
    ...
    pairs = []
    if len(fields) >= 2:
        pairs.append((fields[-2], fields[-1]))          # ..._YYMMDD_HHMMSS
    if fields:
        for sep in ("-", "T", "."):                     # ..._YYYYMMDD-HHMMSS
            if sep in fields[-1]:
                parts = fields[-1].split(sep)
                if len(parts) == 2:
                    pairs.append((parts[0], parts[1]))

    for date_field, time_field in pairs:
        stamp = _to_stamp(date_field, time_field)
