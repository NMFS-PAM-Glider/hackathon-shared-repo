...
def rewind(source):
    """Put a file-like source back at byte 0 before handing it to a reader.

    A Streamlit upload is one object reused for every call in a rerun, so the
    header probe leaves its read pointer past the RIFF header. Handing that
    same object straight to libsndfile makes it start mid-file and report
    "Format not recognised" for a perfectly good wav. Paths are untouched:
    they are reopened on every call anyway.
    """
    if hasattr(source, "seek"):
        try:
            source.seek(0)
        except (OSError, ValueError):  # a closed or non-seekable stream
            pass
    return source
...
def wav_info(source) -> dict:
    ...
    with wave.open(rewind(source), "rb") as w:
...
def read_wav(source, start_s: float = 0.0, duration_s: float | None = None):
    ...
    """
    if sf is not None:
        with sf.SoundFile(rewind(source)) as f:
            sr = int(f.samplerate)
